...
class SingleExpansionActivity : ComponentActivity() {
    var idString: String? = null

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_single_expansion)

        val nameText = findViewById<TextView>(R.id.expansionTitle)
        val typeText = findViewById<TextView>(R.id.expansionType)
        val yearText = findViewById<TextView>(R.id.expansionYear)
        val idText = findViewById<TextView>(R.id.expansionID)

        idString = getIntent().getStringExtra("id")
        val id = idString?.toInt()

        val dbHandler = MyDBHandler(this, null, null, 1)
        val currentGame = dbHandler.findExpansion(id)

        nameText.text = currentGame?.name
        typeText.text = "dodatek"
        ...
        val photoButton: Button = findViewById(R.id.cameraButton1)
        val galleryButton: Button = findViewById(R.id.galleryButton1)
    ...
    fun displayPictures() {
        ...
        if(currentDirectory.exists()) {
            ...
            val photoLayout: LinearLayout = findViewById(R.id.PhotoLayout1)
